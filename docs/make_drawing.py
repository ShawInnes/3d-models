...
Sheets 1, 3, 4, 5 are A3 landscape at 1:1 (readable at any print scale).
Sheet 2 is a schedule. Sheet 6 is an A4 1:1 overlay that MUST print at 100 %.
...
class Sheet:
    ...
    def text(self, x, y, s, size=7, anchor="l", rot=0, col=(0, 0, 0), bold=False):
        c = self.c; c.saveState(); c.setFillColorRGB(*col)
...
def sheet_pcb(c, n, of):
    ...
    for ref, edge, p, w, form, a, b, name in CONNECTORS:
        ...
        how = ("bored %.2f dia, axis %.2f above the PCB top face" % (a, b)
               if form == "round" else "slot %.2f to %.2f about the PCB top face" % (a, b))
...
def sheet_table(c, n, of):
    ...
    for o in openings():
        size = "%.2f dia" % o["d"] if o["form"] == "round" else "%.2f wide" % o["w"]
    ...
    s.text(x1, y1, "face: a %.2f x %.2f block, %.2f up and %.2f down off the board."
           % (SMA_BODY_W, SMA_BLOCK_H, SMA_UP, SMA_DOWN), 7); y1 -= 10
    ...
    y1 -= 5
    s.text(x1, y1, "Overall %.2f x %.2f x %.2f" % (OUT_X, OUT_Y, TOTAL_H), 8, bold=True)
    y1 -= 5.2
    s.text(x1, y1, "Cavity %.2f x %.2f,  wall %.2f,  corner R%.2f"
           % (CAV_X, CAV_Y, WALL, CORNER_R), 8)
    y1 -= 5.2
    s.text(x1, y1, "Outer edge chamfers  bottom %.2f, top %.2f, both 45 deg"
           % (CH_BOT, CH_TOP), 8)
    y1 -= 5.2
    s.text(x1, y1, "Fasteners  4 x %s x %.0f through lid into %s inserts, bore %.2f x %.2f deep"
           % (SCREW_NAME, SCREW_LEN, SCREW_NAME, INSERT_D, INSERT_H), 8)
    y1 -= 5.2
    s.text(x1, y1, "Head recess %.2f dia x %.2f deep, seat z %.2f, %.2f of thread in the insert"
           % (HEAD_D, CBORE_H, SEAT_Z, ENGAGE), 8)
    c.showPage()
...
def case_plan(c, n, of, which):
    ...
    for o in openings():
        ...
        how = ("%.2f dia bore, axis z %.2f" % (o["d"], o["zc"]) if o["form"] == "round"
...
def sheet_section(c, n, of):
    ...
    for o in openings():
        ...
        if o["form"] == "round":
            ...
            s.circle(X(o["c"]), Y(o["zc"]), o["d"]*k, MED, RED)
            s.cross(X(o["c"]), Y(o["zc"]), 5, RED)
            s.text(X(o["c"]), Y(o["z1"])+2.5, "%s  %.2f dia" % (o["ref"], o["d"]),
                   7, "c", col=RED)
        else:
    ...
    s.text(22, 62, "C3 and C4 bored %.2f dia about the SMA block centre, %.2f above the PCB "
                   "top face. Grey dashed outline is the %.2f x %.2f block, %.2f above and "
                   "%.2f below the board."
           % (SMA_HOLE_D, SMA_AXIS_Z, SMA_BODY_W, SMA_BLOCK_H, SMA_UP, SMA_DOWN), 7.5)
    c.showPage()
...
def main(path):
    ...
    sheet_pcb(c, 1, 6); sheet_table(c, 2, 6)
    case_plan(c, 3, 6, "base"); case_plan(c, 4, 6, "lid")
    sheet_section(c, 5, 6); sheet_overlay(c, 6, 6)
    c.save()
