...
One model, `10inch-rack.scad`, generates the face plate, the chassis that wraps the device, the
rack mounting slots, ventilation, zip-tie retention points, and optional keystone jack ports. Bay
count, device dimensions and rack height are all parameters — the layout re-solves itself around
them.

Two ready-to-print STLs are in [`stl/`](stl/).
...
OpenSCAD (2021.01 or newer; the Manifold backend is the default in 2025.x and is what these
instructions assume). Parameters are grouped into Customizer sections, so the GUI Customizer panel
works as well as `-D` on the command line.
...
[docs/DESIGN.md](docs/DESIGN.md); the thin parts are in-plane strips beside the rack slots and
below the jacks, which is what plate thickness and `keystone_clearance` are protecting.

## Verifying changes

The F5 preview is OpenCSG. It invents artifacts that aren't in the model and hides errors that are.
Prove geometry changes with an exact render instead, across every parameter combination the change
touches:
...
Note that `Status: NoError` is necessary but not sufficient: the Manifold backend can emit vertex
pairs nanometres apart that collapse when a slicer loads them as float32, surfacing as
"non-manifold edges" there. [CLAUDE.md](CLAUDE.md) covers the full set of rules — flush cut planes,
coplanar faces inside nested booleans, and how to check what a slicer will see.

## Docs

- [docs/DESIGN.md](docs/DESIGN.md) — width and vertical budgets, load path, lip engagement, cooling,
  known constraints, all measured from exported STL vertices rather than screenshots.
- [docs/KEYSTONE-FIT.md](docs/KEYSTONE-FIT.md) — the keystone aperture against a real Cat6A coupler,
  with a reusable probe harness.
- [CLAUDE.md](CLAUDE.md) — working rules for modifying the model.
</content>
